...
StocksDB.prototype.updateRanking = function(cb) {
	...
	this.query('SET @rank := 0; REPLACE INTO ranking(`type`,uid,rank) SELECT "following", id, @rank := @rank + 1 FROM users WHERE deletiontime IS NULL AND totalfperfbase != 0 ORDER BY tradecount > 0 DESC, (dayfperfcur+totalfperfsold) / totalfperfbase DESC', [], function() {
	...
	this.query('SET @rank := 0; REPLACE INTO ranking(`type`,uid,rank) SELECT "following-week", id, @rank := @rank + 1 FROM users WHERE deletiontime IS NULL AND totalfperfbase != 0 ORDER BY tradecount > 0 DESC, (dayfperfcur+weekfperfsold) / weekfperfbase DESC', [], function() {
