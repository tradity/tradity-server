...
StocksDB.prototype.updateRanking = function(cb) {
	...
	this.query('SET @rank := 0; REPLACE INTO ranking(`type`,`group`,uid,rank) SELECT "following", "students", id, @rank := @rank + 1 FROM users WHERE deletiontime IS NULL AND school IS NOT NULL AND totalfperfbase != 0 ORDER BY (dayfperfcur+totalfperfsold) / totalfperfbase DESC', [], function() {
