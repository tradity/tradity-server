...
/**
 * Parses and provides the server configuration.
 * Currently, all config file loading is done upon
...
let otherConfigFiles;

/**
 * Reload all config files.
 * 
 * @function module:config~reloadConfig
 */
function reloadConfig() {
  debug('Reloading config');
  
  cfg = require('./config/config.global.js');
  
  otherConfigFiles = getConfigFiles(cfg).filter(fs.existsSync);

  for (let i = 0; i < otherConfigFiles.length; ++i) {
    let cfgFileContent = {};
    
    debug('Loading config file', otherConfigFiles[i]);
    try {
      cfgFileContent = require(otherConfigFiles[i]);
    } catch (e) { debug('Failure loading config file', otherConfigFiles[i], e); }
    
    cfg = deepupdate(cfg, cfgFileContent);
  }

  cfg.varReplace = function(s) { return s.replace(/\{\$(.+?)\}/g, function(match, varname) { return cfg[varname]; }); };
}
...
if (require.main === module) {
  ...
  if (options['show-files']) {
    for (let i = 0; i < otherConfigFiles.length; ++i) {
      console.log(otherConfigFiles[i]);
    }
  } else if (path.length > 0) {
    console.log(_.reduce(path, _.result, cfg));
  } else {
    console.log('Config files:', otherConfigFiles);
